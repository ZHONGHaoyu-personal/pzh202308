...
// 省份在左侧/右侧的分布
const leftProvinces = ['陕西', '湖北', '四川', '湖南', '重庆', '广西', '海南'];
const rightProvinces = ['黑龙江', '内蒙古', '辽宁', '吉林', '北京', '天津', '山西', '山东', '江苏', '安徽', '上海', '浙江'];
...
function updateStats() {
    ...
    const universities = new Set(filteredData.students.filter(s => s.university && s.university !== '待补充').map(s => s.university));
    document.getElementById('totalUniversities').textContent = universities.size;

    const provinces = new Set(filteredData.students.filter(s => s.province && s.province !== '待补充').map(s => s.province));
